...
class Furk:
	...
	def search_new(self):
		...
		try:
			...
			dbcur.close() ; dbcon.close()
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.kingpin/?action=furk_searchResults&query=%s,return)' % quote_plus(query))
		...
		for count, item in enumerate(files, 1):
			try:
				if item['is_ready'] == '1' and item['type'] == 'video':
					...
					cm.append((downloadMenu, 'RunPlugin(plugin://plugin.video.kingpin/?action=download&name=%s&image=%s&url=%s&caller=furk)' %
