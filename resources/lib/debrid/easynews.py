...
class EasyNews:
	...
	def search_new(self):
		...
		try:
			...
			dbcur.close() ; dbcon.close()
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.kingpin/?action=en_searchResults&query=%s,return)' % quote_plus(query))
	...
	def query_results_to_dialog(self, query):
		...
		for count, item in enumerate(files, 1):
			try:
				...
				url = 'plugin://plugin.video.kingpin/?action=en_resolve_forPlayback&url=%s' % sysurl_dl
				cm.append((downloadMenu, 'RunPlugin(plugin://plugin.video.kingpin/?action=download&name=%s&image=%s&url=%s&caller=easynews)' %
