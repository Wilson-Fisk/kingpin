...
##################
	...
	def keepAlive(self):
		...
		if playerWindow.getProperty('kingpin.playlistStart_position'): pass
		else:
			if control.playlist.size() > 1: playerWindow.setProperty('kingpin.playlistStart_position', str(control.playlist.getposition()))
						...
						if self.enable_playnext and not self.play_next_triggered:
							if int(control.playlist.size()) > 1:
								if self.preScrape_triggered == False:
									xbmc.executebuiltin('RunPlugin(plugin://plugin.video.kingpin/?action=play_preScrapeNext)')
								...
								if remaining_time < (self.playnext_time + 1) and remaining_time != 0:
									xbmc.executebuiltin('RunPlugin(plugin://plugin.video.kingpin/?action=play_nextWindowXML)')
...
### Kodi player callback methods ###
	def onAVStarted(self): # Kodi docs suggests "Use onAVStarted() instead of onPlayBackStarted() as of v18"
		...
		xbmc.log('[ plugin.video.kingpin ] onAVStarted callback', LOGINFO)
		log_utils.log('[ plugin.video.kingpin ] onAVStarted callback', level=log_utils.LOGDEBUG)

	def onPlayBackSeek(self, time, seekOffset):
		seekOffset /= 1000

	def onPlayBackSeekChapter(self, chapter):
		log_utils.log('[ plugin.video.kingpin ] onPlayBackSeekChapter callback', level=log_utils.LOGDEBUG)

	def onQueueNextItem(self):
		log_utils.log('[ plugin.video.kingpin ] onQueueNextItem callback', level=log_utils.LOGDEBUG)

	def onPlayBackStopped(self):
		try:
			playerWindow.clearProperty('kingpin.preResolved_nextUrl')
			playerWindow.clearProperty('kingpin.playlistStart_position')
			clear_local_bookmarks() # clear all kingpin bookmarks from kodi database
			...
			if not self.onPlayBackStopped_ran or (self.playbackStopped_triggered and not self.onPlayBackStopped_ran): # Kodi callback unreliable and often not issued
				...
				if getSetting('crefresh') == 'true' and seekable:
					log_utils.log('[ plugin.video.kingpin ] container.refresh issued', level=log_utils.LOGDEBUG)
				...
				xbmc.log('[ plugin.video.kingpin ] onPlayBackStopped callback', LOGINFO)
				log_utils.log('[ plugin.video.kingpin ] onPlayBackStopped callback', level=log_utils.LOGDEBUG)
		except: log_utils.error()
	...
	def onPlayBackEnded(self):
		...
		if control.playlist.getposition() == control.playlist.size() or control.playlist.size() == 1:
			control.playlist.clear()
		xbmc.log('[ plugin.video.kingpin ] onPlayBackEnded callback', LOGINFO)
		log_utils.log('[ plugin.video.kingpin ] onPlayBackEnded callback', level=log_utils.LOGDEBUG)
	...
	def onPlayBackError(self):
		...
		Bookmarks().reset(self.current_time, self.media_length, self.name, self.year)
		log_utils.error()
		xbmc.log('[ plugin.video.kingpin ] onPlayBackError callback', LOGINFO)
		log_utils.log('[ plugin.video.kingpin ] onPlayBackError callback', level=log_utils.LOGDEBUG)
		sysexit(1)
##############################
...
class PlayNext(xbmc.Player):
	...
	def isStill_watching(self):
		...
		playlistStart_position = int(playerWindow.getProperty('kingpin.playlistStart_position'))
