...
DEBUGPREFIX = '[COLOR red][ kingpin: %s ][/COLOR]'
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		if debug_location == '1':
			log_file = joinPath(LOGPATH, 'kingpin.log')
		...
		traceback.print_exc()
		import xbmc
		xbmc.log('[ plugin.video.kingpin ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
def error(message=None, exception=True):
	try:
		...
		import xbmc
		xbmc.log('[ plugin.video.kingpin ] log_utils.error() Logging Failure: %s' % (e), LOGERROR)
...
def clear_logFile():
	...
	try:
		...
		import xbmc
		xbmc.log('[ plugin.video.kingpin ] log_utils.clear_logFile() Failure: %s' % (e), LOGERROR)
		cleared = False
	return cleared
...
def view_LogFile(name):
	try:
		...
		windows = TextViewerXML('textviewer.xml', addonPath('plugin.video.kingpin'), heading=heading, text=text)
		windows.run()
		del windows
	except: error()
...
def upload_LogFile(name):
	...
	try:
		...
		text = f.read()
		f.close()
		UserAgent = 'kingpin %s' % addonVersion('plugin.video.kingpin')
		...
		if 'key' in response.json():
			...
			select = selectDialog([i[0] for i in list], lang(32196) if name.lower() == 'kingpin' else lang(32199))
