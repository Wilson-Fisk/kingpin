...
from kingpinscrapers import sources as fs_sources
...
class Sources:
	...
	def play(self, title, year, imdb, tmdb, tvdb, season, episode, tvshowtitle, premiered, meta, select, rescrape=None):
		...
		try:
			control.sleep(200)
			if control.playlist.getposition() == 0 or control.playlist.size() == 1: playerWindow.clearProperty('kingpin.preResolved_nextUrl')
			preResolved_nextUrl = playerWindow.getProperty('kingpin.preResolved_nextUrl')
			if preResolved_nextUrl != '':
				control.sleep(500)
				playerWindow.clearProperty('kingpin.preResolved_nextUrl')
...
##################
		...
		try:
			...
			if action == 'play_Item' and self.uncached_chosen != True:
				return self.playItem(title, items, chosen_source.getProperty('kingpin.source_dict'), self.meta)
			else:
				control.cancelPlayback()
		except:
	...
	def preResolve(self, next_sources, next_meta):
		try:
			...
			log_utils.error()
			return playerWindow.clearProperty('kingpin.preResolved_nextUrl')
				...
				try:
					...
					if url:
						...
						if player_hasVideo: # do not setPropery if user stops playback quickly because "onPlayBackStopped" is already called and won't be able to clear it.
							playerWindow.setProperty('kingpin.preResolved_nextUrl', url)
	...
	def getConstants(self):
		self.metaProperty = 'plugin.video.kingpin.container.meta'
		self.seasonProperty = 'plugin.video.kingpin.container.season'
		self.episodeProperty = 'plugin.video.kingpin.container.episode'
		self.titleProperty = 'plugin.video.kingpin.container.title'
		self.imdbProperty = 'plugin.video.kingpin.container.imdb'
		self.tmdbProperty = 'plugin.video.kingpin.container.tmdb'
		self.tvdbProperty = 'plugin.video.kingpin.container.tvdb'
		self.labelProperty = 'plugin.video.kingpin.container.label'
		...
		if control.addon('script.module.kingpinscrapers').getSetting('easynews.user'): self.prem_providers += [('easynews', int(getSetting('easynews.priority')))]
		if control.addon('script.module.kingpinscrapers').getSetting('filepursuit.api'): self.prem_providers += [('filepursuit', int(getSetting('filepursuit.priority')))]
		if control.addon('script.module.kingpinscrapers').getSetting('furk.user_name'): self.prem_providers += [('furk', int(getSetting('furk.priority')))]
		if control.addon('script.module.kingpinscrapers').getSetting('gdrive.cloudflare_url'): self.prem_providers += [('gdrive', int(getSetting('gdrive.priority')))]
		if control.addon('script.module.kingpinscrapers').getSetting('ororo.user'): self.prem_providers += [('ororo', int(getSetting('ororo.priority')))]
