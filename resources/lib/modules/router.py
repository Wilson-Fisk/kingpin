...
def router(params):
	...
	if action is None:
		...
		isUpdate = control.homeWindow.getProperty('kingpin.updated')
		if isUpdate == 'true':
			control.execute('RunPlugin(plugin://plugin.video.kingpin/?action=tools_cleanSettings)')
			control.homeWindow.clearProperty('kingpin.updated')
			from resources.lib.modules import changelog
			changelog.get('kingpin')
	...
	####################################################
		...
		if action == 'tools_ShowNews':
			...
			control.openSettings('0.0', 'context.kingpin')
			control.trigger_widget_refresh()
		elif action == 'tools_kingpinscrapersSettings':
			...
			elif cleared: control.notification(message='kingpin Log File Successfully Cleared')
			else: control.notification(message='Error clearing kingpin Log File, see kodi.log for more info')
	...
	####################################################
		...
		if action == 'play_Item':
			...
			for i in items:
				...
				url = 'plugin://plugin.video.kingpin/?action=play_Item&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&select=1' % (
			...
			if rtype == 'movie':
				...
				r = 'plugin://plugin.video.kingpin/?action=play_Item'
			elif rtype == 'episode':
				from resources.lib.menus import episodes
				...
				r = 'plugin://plugin.video.kingpin/?action=play_Random&rtype=episode'
			elif rtype == 'show':
				from resources.lib.menus import tvshows
				rlist = tvshows.TVshows().get(url, create_directory=False)
				r = 'plugin://plugin.video.kingpin/?action=play_Random&rtype=season'
	...
	####################################################
		...
		elif action == 'cache_clearKodiBookmark': # context.kingpin action call only
