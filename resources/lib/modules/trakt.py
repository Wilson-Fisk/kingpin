...
V2_API_KEY = '8cae0aaf79eba69f72f0ae645cf552dfaa750dec451b296921c2a18678a1d975'
CLIENT_SECRET = '632740805b4e64ca7d6f9ec17754ba9886281f5a2e527713c27b320bcaaf2221'
...
def unHideItems(tvdb_ids):
	...
	try:
		...
		for section in sections:
			...
			control.sleep(1000)
		if success:
			if 'plugin.video.kingpin' in control.infoLabel('Container.PluginName'): control.refresh()
	...
	try:
		...
		if select >= 0:
			if items[select][1] == 'watch':
				...
				control.execute('RunPlugin(plugin://plugin.video.kingpin/?action=shows_traktHiddenManager)')
			elif items[select][1] == 'unfinishedEpisodeManager':
				control.execute('RunPlugin(plugin://plugin.video.kingpin/?action=episodes_traktUnfinishedManager)')
			elif items[select][1] == 'unfinishedMovieManager':
				control.execute('RunPlugin(plugin://plugin.video.kingpin/?action=movies_traktUnfinishedManager)')
