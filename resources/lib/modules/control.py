...
def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('kingpin_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('kingpin_settings', jsdumps(settings_dict))
		refresh_playAction()
		refresh_libPath()
...
def getkingpinVersion():
	return xbmcaddon.Addon('plugin.video.kingpin').getAddonInfo('version')
...
def artPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.kingpin').getAddonInfo('path'), 'resources', 'artwork', theme)

def genreIconPath():
	return joinPath(xbmcaddon.Addon('plugin.video.kingpin').getAddonInfo('path'), 'resources', 'artwork', 'genre_media', 'icons')

def genrePosterPath():
	return joinPath(xbmcaddon.Addon('plugin.video.kingpin').getAddonInfo('path'), 'resources', 'artwork', 'genre_media', 'posters')
...
def refresh_playAction(): # for kingpin global CM play actions
	autoPlay = 'true' if setting('play.mode') == '1' else ''
	homeWindow.setProperty('kingpin.autoPlay.enabled', autoPlay)

def refresh_libPath(): # for kingpin global CM library actions
	homeWindow.setProperty('kingpin.movieLib.path', transPath(setting('library.movie')))
	homeWindow.setProperty('kingpin.tvLib.path', transPath(setting('library.tv')))

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear kingpin.log if setting to reverse has been changed
	if homeWindow.getProperty('kingpin.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('kingpin.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://plugin.video.kingpin/?action=tools_clearLogFile)')
