...
class Movies:
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.kingpin/?action=movies&url=%s,return)' % (quote_plus(url)))
	...
	def person(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.kingpin/?action=moviePersons&url=%s,return)' % (quote_plus(url)))
