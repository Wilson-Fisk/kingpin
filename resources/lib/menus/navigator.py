...
class Navigator:
	...
	def root(self):
		...
		if getMenuEnabled('navi.changelog'): self.addDirectoryItem(32014, 'tools_ShowChangelog&name=kingpin', 'icon.png', 'DefaultAddonHelper.png', isFolder=False)
		self.endDirectory()
	...
	def traktSearchLists(self, media_type):
		...
		control.closeAll()
		if media_type == 'movies': control.execute('ActivateWindow(Videos,plugin://plugin.video.kingpin/?action=movies_PublicLists&url=%s,return)' % (quote_plus(url)))
		else: control.execute('ActivateWindow(Videos,plugin://plugin.video.kingpin/?action=tv_PublicLists&url=%s,return)' % (quote_plus(url)))
	...
	def loggingNavigator(self):
		self.addDirectoryItem(32524, 'tools_viewLogFile&name=kingpin', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
	...
	def views(self):
		try:
			...
			title = getLS(32059)
			url = 'plugin://plugin.video.kingpin/?action=tools_addView&content=%s' % content
	...
	def addDirectoryItem(self, name, query, poster, icon, context=None, queue=False, isAction=True, isFolder=True, isPlayable=False, isSearch=False, table=''):
		try:
			...
			url = 'plugin://plugin.video.kingpin/?action=%s' % query if isAction else query
			...
			if queue: cm.append((queueMenu, 'RunPlugin(plugin://plugin.video.kingpin/?action=playlist_QueueItem)'))
			if context: cm.append((getLS(context[0]), 'RunPlugin(plugin://plugin.video.kingpin/?action=%s)' % context[1]))
			if isSearch: cm.append(('Clear Search Phrase', 'RunPlugin(plugin://plugin.video.kingpin/?action=cache_clearSearchPhrase&source=%s&name=%s)' % (table, quote_plus(name))))
			cm.append(('[COLOR red]kingpin Settings[/COLOR]', 'RunPlugin(plugin://plugin.video.kingpin/?action=tools_openSettings)'))
