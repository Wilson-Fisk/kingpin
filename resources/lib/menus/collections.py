...
class Collections:
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.kingpin/?action=collections&url=%s,return)' % (quote_plus(url)))
...
#################################
	...
	def movieDirectory(self, items, next=True):
		...
		sysaddon, syshandle = 'plugin://plugin.video.kingpin/', int(argv[1])
...
####-Context Menu and Overlays-####
				...
				cm.append(('[COLOR red]kingpin Settings[/COLOR]', 'RunPlugin(%s?action=tools_openSettings)' % sysaddon))
####################################
				...
				if is_widget: item.setProperty('iskingpin_widget', 'true')
	...
	def addDirectoryItem(self, name, action, poster, icon, fanart=None, context=None, queue=False):
		try:
			...
			url = 'plugin://plugin.video.kingpin/?action=%s' % action
			cm = []
