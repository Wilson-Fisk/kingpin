...
class TVshows:
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.kingpin/?action=tvshows&url=%s,return)' % (quote_plus(url)))
	...
	def person(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.kingpin/?action=tvPersons&url=%s,return)' % (quote_plus(url)))
