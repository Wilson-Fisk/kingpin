...
class SourceResultsXML(BaseDialog):
			...
			if action_id in self.selection_actions:
				...
				if 'UNCACHED' in source:
					...
					self.execute_code('RunPlugin(plugin://plugin.video.kingpin/?action=cacheTorrent&caller=%s&type=%s&title=%s&items=%s&url=%s&source=%s&meta=%s)' %
											(debrid, link_type, sysname, quote_plus(jsdumps(self.results)), quote_plus(chosen_source.getProperty('kingpin.url')), quote_plus(source_dict), quote_plus(jsdumps(self.meta))))
					self.selected = (None, '')
				else:
				...
				if cm_action == 'sourceInfo':
					...
					debrid = chosen_source.getProperty('kingpin.debrid')
					name = chosen_source.getProperty('kingpin.name')
					hash = chosen_source.getProperty('kingpin.hash')
					self.execute_code('RunPlugin(plugin://plugin.video.kingpin/?action=showDebridPack&caller=%s&name=%s&url=%s&source=%s)' %
									(quote_plus(debrid), quote_plus(name), quote_plus(chosen_source.getProperty('kingpin.url')), quote_plus(hash)))
					self.selected = (None, '')
					...
					self.execute_code('RunPlugin(plugin://plugin.video.kingpin/?action=download&name=%s&image=%s&source=%s&caller=sources&title=%s)' %
										(new_sysname, quote_plus(poster), quote_plus(source_dict), sysname))
					self.selected = (None, '')
				elif cm_action == 'saveToCloud':
					magnet = chosen_source.getProperty('kingpin.url')
					if debrid == 'AD':
	...
	def set_properties(self):
		...
		try:
			...
			self.setProperty('kingpin.highlight.color', getSourceHighlightColor())
		except:
			from resources.lib.modules import log_utils
			log_utils.error()
