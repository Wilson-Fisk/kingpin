...
class PlayNextXML(BaseDialog):
	...
	def set_properties(self):
		...
		try:
			...
			self.setProperty('kingpin.tvshowtitle', self.meta.get('tvshowtitle'))
			self.setProperty('kingpin.title', self.meta.get('title'))
			...
			self.setProperty('kingpin.episode', str(self.meta.get('episode', '')))
			self.setProperty('kingpin.rating', str(self.meta.get('rating', '')))
			self.setProperty('kingpin.landscape', self.meta.get('landscape', ''))
			self.setProperty('kingpin.fanart', self.meta.get('fanart', ''))
			self.setProperty('kingpin.thumb', self.meta.get('thumb', ''))
			next_duration = int(self.meta.get('duration')) if self.meta.get('duration') else ''
			self.setProperty('kingpin.duration', str(int(next_duration)))
			endtime = (datetime.now() + timedelta(seconds=next_duration)).strftime('%I:%M %p').lstrip('0') if next_duration else ''
			self.setProperty('kingpin.endtime', endtime)
		except:
			from resources.lib.modules import log_utils
			log_utils.error()