...
class CheckSettingsFile:
	def run(self):
		try:
			control.log('[ plugin.video.kingpin ]  CheckSettingsFile Service Starting...', LOGINFO)
			window.clearProperty('kingpin_settings')
			...
			return control.log('[ plugin.video.kingpin ]  Finished CheckSettingsFile Service', LOGINFO)
		except:
			log_utils.error()
...
class SettingsMonitor(control.monitor_class):
	def __init__ (self):
		...
		window.setProperty('kingpin.debug.reversed', control.setting('debug.reversed'))
		control.log('[ plugin.video.kingpin ]  Settings Monitor Service Starting...', LOGINFO)
...
class ReuseLanguageInvokerCheck:
	def run(self):
		control.log('[ plugin.video.kingpin ]  ReuseLanguageInvokerCheck Service Starting...', LOGINFO)
		try:
			import xml.etree.ElementTree as ET
			from resources.lib.modules.language_invoker import gen_file_hash
			addon_xml = control.joinPath(control.addonPath('plugin.video.kingpin'), 'addon.xml')
			tree = ET.parse(addon_xml)
			root = tree.getroot()
			current_addon_setting = control.addon('plugin.video.kingpin').getSetting('reuse.languageinvoker')
			try: current_xml_setting = [str(i.text) for i in root.iter('reuselanguageinvoker')][0]
			except: return control.log('[ plugin.video.kingpin ]  ReuseLanguageInvokerCheck failed to get settings.xml value', LOGINFO)
			...
			if current_xml_setting == current_addon_setting:
				return control.log('[ plugin.video.kingpin ]  ReuseLanguageInvokerCheck Service Finished', LOGINFO)
			...
			for item in root.iter('reuselanguageinvoker'):
				...
				control.log('[ plugin.video.kingpin ]  ReuseLanguageInvokerCheck Service Finished', LOGINFO)
...
class AddonCheckUpdate:
	def run(self):
		control.log('[ plugin.video.kingpin ]  Addon checking available updates', LOGINFO)
		try:
			import re
			import requests
			repo_xml = requests.get('https://raw.githubusercontent.com/123kingpin/repository.kingpin/main/matrix/zips/addons.xml')
			if not repo_xml.status_code == 200:
				return control.log('[ plugin.video.kingpin ]  Could not connect to remote repo XML: status code = %s' % repo_xml.status_code, LOGINFO)
			repo_version = re.findall(r'<addon id=\"plugin.video.kingpin\".+version=\"(\d*.\d*.\d*)\"', repo_xml.text)[0]
			local_version = control.getkingpinVersion()[:5] # 5 char max so pre-releases do try to compare more chars than github version
			...
			if check_version_numbers(local_version, repo_version):
				while control.condVisibility('Library.IsScanningVideo'):
					control.sleep(10000)
				control.log('[ plugin.video.kingpin ]  A newer version is available. Installed Version: v%s, Repo Version: v%s' % (local_version, repo_version), LOGINFO)
				control.notification(message=control.lang(35523) % repo_version)
			return control.log('[ plugin.video.kingpin ]  Addon update check complete', LOGINFO)
		except:
			log_utils.error()
...
class VersionIsUpdateCheck:
	def run(self):
		try:
			...
			if isUpdate:
				window.setProperty('kingpin.updated', 'true')
				curVersion = control.getkingpinVersion()
				...
				if do_cacheClear:
					...
					if cleared:
						...
						control.log('[ plugin.video.kingpin ]  Forced traktsync clear for version update complete.', LOGINFO)
					if clr_fanarttv:
						...
						control.log('[ plugin.video.kingpin ]  Forced fanarttv.db clear for version update complete.', LOGINFO)
				control.setSetting('trakt.message2', '') # force a settings write for any added settings that may have been added in new version
				control.log('[ plugin.video.kingpin ]  Forced new User Data settings.xml saved', LOGINFO)
				control.log('[ plugin.video.kingpin ]  Plugin updated to v%s' % curVersion, LOGINFO)
		except:
			log_utils.error()

class SyncTraktCollection:
	def run(self):
		control.log('[ plugin.video.kingpin ]  Trakt Collection Sync Starting...', LOGINFO)
		...
		control.log('[ plugin.video.kingpin ]  Trakt Collection Sync Complete', LOGINFO)

class LibraryService:
	def run(self):
		control.log('[ plugin.video.kingpin ]  Library Update Service Starting (Update check every 6hrs)...', LOGINFO)
...
class SyncTraktService:
	def run(self):
		...
		control.log('[ plugin.video.kingpin ]  Trakt Sync Service Starting (sync check every %s minutes)...' % service_syncInterval, LOGINFO)
...
def main():
	while not control.monitor.abortRequested():
		control.log('[ plugin.video.kingpin ]  Service Started', LOGINFO)
		schedTrakt = None
		libraryService = None
	...
	control.log('[ plugin.video.kingpin ]  Settings Monitor Service Stopping...', LOGINFO)
	del syncTraktService # prob does not kill a running thread
	control.log('[ plugin.video.kingpin ]  Trakt Sync Service Stopping...', LOGINFO)
	if libraryService:
		del libraryService # prob does not kill a running thread
		control.log('[ plugin.video.kingpin ]  Library Update Service Stopping...', LOGINFO)
	if schedTrakt:
		schedTrakt.cancel()
	control.log('[ plugin.video.kingpin ]  Service Stopped', LOGINFO)
